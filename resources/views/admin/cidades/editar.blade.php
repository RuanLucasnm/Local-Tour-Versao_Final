...
@section('title', 'Editar Cidade - Local Tour')
...
<div class="form-container">
    <h1>✏️ Editar Cidade</h1>

    <form method="POST" action="/admin/cidades/{{ $cidade->id_cidade }}">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="nome">Nome *</label>
            <input type="text" id="nome" name="nome" value="{{ old('nome', $cidade->nome) }}" required placeholder="Ex: Niterói">
            @error('nome')
        ...
        <div class="form-group">
            ...
            <input type="text" id="estado" name="estado" value="{{ old('estado', $cidade->estado) }}" required placeholder="Ex: RJ">
            @error('estado')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-submit">Atualizar Cidade</button>
